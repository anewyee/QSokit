...
void BaseForm::send()
{
    ...
    qDebug()<< __FUNCTION__ <<__LINE__<< e->text();

    QString err;
    QByteArray bin;
    if (!TK::ascii2bin(e->text(), bin, err))
    {
        qDebug()<< __FUNCTION__ <<__LINE__<< "bad data format to send: "+err;

        return;
    }

    qDebug()<< __FUNCTION__ <<__LINE__<< bin;

    QString hex = TK::bin2hexOnly(bin,m_cntSend);
    qDebug()<< __FUNCTION__ <<__LINE__<< hex << m_cntSend;
}
